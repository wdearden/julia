...
function finish(me::InferenceState)
    ...
    if me.limited && me.cached && me.parent !== nothing
        ...
        type_annotate!(me)
        if me.cached || me.parent !== nothing
            # construct the optimizer for later use
...
function type_annotate!(sv::InferenceState)
    run_optimizer = sv.cached # delete dead statements only if we're building this IR to cache it
