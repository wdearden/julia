...
function print_ssa(io::IO, val, argnames)
    if isa(val, SSAValue)
        Base.print(io, "%$(val.id)")
    elseif isa(val, Argument)
        Base.print(io, isempty(argnames) ? "%%$(val.n)" : "%%$(argnames[val.n])")
    else
        try
            isa(val, GlobalRef) ? Base.print(io, val) : Base.show(io, val)
        catch
...
function print_node(io::IO, idx, stmt, used, argnames, maxsize; color = true, print_typ=true)
    ...
    elseif isexpr(stmt, :invoke)
        ...
        Base.print(io, "(")
        print_arg((typ, arg)) = sprint(io->(print_ssa(io, arg, argnames); print(io, "::$(typ)")))
        Base.print(io, join(map(print_arg, zip(linfo.specTypes.parameters[2:end], stmt.args[3:end])), ", "))
        Base.print(io, ")")
        if print_typ && stmt.typ !== Any
            Base.print(io, "::$(stmt.typ)")
        end
    elseif isexpr(stmt, :new)
        Base.print(io, "new(")
        Base.print(io, join(map(arg->sprint(io->print_ssa(io, arg, argnames)), stmt.args), ", "))
        Base.print(io, ")")
    else
        Base.print(io, stmt)
    end
...
function show_ir(io::IO, code::IRCode, expr_type_printer=default_expr_type_printer; argnames=Symbol[], verbose_linetable=get(io, :verbose_linetable, false))
