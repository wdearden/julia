...
function analyze_method!(idx, f, ft, metharg, methsp, method, stmt, atypes, sv, atype_unlimited, isinvoke, isapply, invoke_data)
    ...
    @timeit "inline IR inflation" if src.codelocs === nothing
        ...
        ir2 = just_construct_ssa(src, ast, na-1, inline_linetable, spvals_from_meth_instance(linfo))
    else
...
function handle_single_case!(ir, stmt, idx, case, isinvoke, todo, sv::OptimizationState)
    if isa(case, ConstantCase)
